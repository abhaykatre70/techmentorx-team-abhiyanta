...
-- =================================================================================
-- MASTER SCRIPT: INITIALIZE DATABASE WITH INDIAN DEMO DATA
-- Run this ENTIRE script in Supabase SQL Editor.
-- =================================================================================

-- 1. CLEANUP (Drop tables if they exist to avoid conflicts)
...
CREATE TABLE public.users (
  id uuid PRIMARY KEY, -- No strict FK to auth.users to allow demo data
  email text,
  name text,
...
-- (F) Storage Buckets (Attempt creation, safe if exists)
INSERT INTO storage.buckets (id, name, public) 
VALUES ('reports', 'reports', true)
ON CONFLICT (id) DO NOTHING;

CREATE POLICY "Public Access Bucket" ON storage.objects FOR SELECT USING (bucket_id = 'reports');
CREATE POLICY "Auth Upload Bucket" ON storage.objects FOR INSERT WITH CHECK (bucket_id = 'reports' AND auth.role() = 'authenticated');
